...
! Generate X2= (I+eig)^{-0.5} * W^T * S
   ...
   integer, intent(in) :: nrobs
   integer, intent(in) :: nrmin
!   real, intent(in)    :: W(nrmin,nrens)  ! Bug reorted by Marco Bajo
...
!   print '(5g11.3)',sig(1:min(nrmin,nrens))
   ...
   call dgemm('n','n',nrens,nrens,nrens,1.0,X3,nrens,ROT,nrens,0.0,X4,nrens)
